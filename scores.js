...
import { getAbilityScores } from "./cache.js";
import randomItem from "random-item";
import { abstractDice, removeSmallest } from "./utils.js";
...
export const getBestScores = async function (race, subrace, classe) {
  let values = [12, 10, 8];
  ...
  switch (classe.data.name) {
    case "Barbarian":
      ...
      abilitiesObj = abilitiesObj.filter(function (obj) {
        return (
          obj?.name !== abone && obj?.name !== abtwo && obj?.name !== abthree
        );
      });
      break;
    case "Bard":
...
export const getRolledScores = async function (race, subrace) {
  let ability = await getAbilityScores();
  let values = [];
  //let rolling = []
  let i;
...
export const getStandardScores = async function (race, subrace) {
