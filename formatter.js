import { replyEscaper } from "./utils.js";

export const getFormattedSpell = function (spell) {
  let Fspell = "<u><b>" + spell.data.name + "</b></u>\n";
  Fspell +=
    spell.data.level +
    " level " +
    spell.data.school.name +
    " " +
    (spell.data.ritual === true ? "(Ritual)" : "") +
    "\n";
  Fspell += "<b>Casting Time: </b>" + spell.data.casting_time + "\n";
  Fspell += "<b>Range: </b>" + spell.data.range + "\n";
  Fspell +=
    "<b>Components: </b>" +
    spell.data.components.join(", ") +
    " (" +
    (typeof spell.data.material === "undefined"
      ? "nessun materiale"
      : spell.data.material) +
    ")\n";
  Fspell +=
    "<b>Duration: </b>" +
    (spell.data.concentration === true ? "Concentration," : "") +
    " " +
    spell.data.duration +
    "\n";
  let classes = "";
  spell.data.classes.forEach((element, index) =>
    index > 0 ? (classes += ", " + element.name) : (classes += element.name)
  );
  Fspell += "<b>Classes: </b>" + classes + "\n";
  Fspell += "<b>Description:\n</b>" + spell.data.desc + "\n";
  Fspell +=
    (typeof spell.data.higher_level === "undefined" ||
    spell?.data?.higher_level.length === 0
      ? ""
      : "<b>Higer Levels:\n</b>" + spell.data.higher_level) + "\n";
  Fspell = replyEscaper(Fspell);
  return Fspell;
};
export const getFormattedReply = function (character) {
  let reply = "<b>NAME: </b>" + character.name;
  reply +=
    "\n<b>CLASS: </b>" +
    character.classe.data.name +
    " " +
    character.level +
    "\n";
  if (character.scores) {
    reply +=
      ...
      "\n<b>CON: </b>" +
      character.scores["CON"] +
      "\n<b>INT: </b>" +
      character.scores["INT"] +
      "\n<b>WIS: </b>" +
      character.scores["WIS"] +
      "\n<b>CHA: </b>" +
      character.scores["CHA"] +
      "\n";
  }
  ...
  reply +=
    "\n<b><u>" +
    character.background.feature.name +
    "</u></b>\n" +
